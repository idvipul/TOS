...
// stores history of the commands typed
...
void welcomeMsg(int window_id) {
wm_print(window_id, "        __      __  __   _ _   __  \n");
wm_print(window_id, " |   | |_  |   |   |  | | | | |_  \n");
wm_print(window_id, " |_|_| |__ |__ |__ |__| |   | |__  \n");
wm_print(window_id, "\n ");
}
...
void string_copy (int window_id, char* history_command, char* new_command) {
  int index = 0;

  while(new_command[index] != '\0') {
    // store value of new_cmd in history_cmd
    history_command[index] = new_command[index];
    index++;
  }

  history_command[index + 1] = '\0';
}

// print all the commands typed -- history
void print_history(int window_id, shell_history* head) {
  int index = 0;
...
void execute_pling_command(int window_id, int history_index, shell_history* head) {
  ...
  while(head->next != NULL) {
    if (index == history_index) {
      ...
      find_shell_command(window_id, head->command, head);
      return;
    }
    index++;
    head = head->next;
...
void execute_shell_command(int window_id, int command_index, shell_history* head) {
  switch(command_index) {
    ...
    break;
    case 6: print_history(window_id, head);
    break;
...
void shell_process(PROCESS self, PARAM param) {
  ...
  while (42) {
    ...
    while(key != 13) {
      ...
      for (int length = command_index - 1; length > 0; length--) { // since last char is \0
    ...
    new_command->command = (char*) malloc((command_index + 1) * sizeof(char));
    string_copy(window_id, new_command->command, command);
